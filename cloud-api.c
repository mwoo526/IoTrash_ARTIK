...
int message_command(int argc, char *argv[],char* artikID ,double trashAmount,int ledColor)
{
	...
	char values[512];
		...
		argc=6; // argument 의 개수
		argv[3]="99f1a336ae524993a148cf00dcc43bb2"; // argv[0] 실행파일 - > 옵션 추가시 +1
		argv[4]="1fe73e3569884457920f5d8347eaa4f7";

		sprintf(values, "{\"trash_Amount\":%lf}", trashAmount);
		argv[5]=values;
		...
		sprintf(values, "{\"led_Color\":%d}" ,ledColor);
		argv[5]=values;
		...
		sprintf(values, "{\"artik_ID\":%s}",artikID);
		argv[5]=values;
